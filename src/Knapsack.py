...
class Player():
    ...
    def __str__(self):
        return "{} {} {} {} {}".format(self.name,self.position,self.salary, round(self.points, 2), round((self.value * 1000), 2))

with open('../data/daily_predictions/for_knapsack.csv','r') as data:
...
def lineup_knapsack_fanduel(players):
    cap = 60000
...
def lineup_knapsack_dk(players):
    cap = 50000
    current_lineup_cap = 0
    limits = {
        'PG':1,
    ...
    for player in players:
        ...
        if counter['G'] < limits['G'] and current_lineup_cap + sal <= cap and pos in ['PG','SG']:
            ...
            current_lineup_cap += sal
            continue
        if counter['F'] < limits['F'] and current_lineup_cap + sal <= cap and pos in ['SF', 'PF']:
            lineup.append(player)
...
lineup = lineup_knapsack_fanduel(players)
points = 0
salary = 0
for player in lineup:
    points += player.points
